...
void AltaNewDron(datoDron dron, datoAlmacen depot ) {
  ...
  while (toupper(dronOpc) != 'N') {
    ...
    BuscaReg(numregistro, auxalmacen,  dron );
    BuscaRef( numRef, dron);
...
int BuscaReg( int & numregistro, int numalmacen, datoDron dron) {
  ...
  while ( ind0 <= dron[ind0].reg_dron) {      // recorremos toda el array hasta encontrar el ultimo numero de dron asignado

    if ( dron[ind0].almacen == numalmacen ) {
      do {
        if ( dron[ind0].idDron > 0) {
          numregistro = dron[ind0].idDron + 1;
        };
        ind0++;
      } while (dron[ind0].almacen == numalmacen);
    } else {
      ;
      ind0++;
    };
...
int BuscaRef (int & numRef, datoDron dron) {
  int ind0 = 0;
  while (ind0 < 101) {

    if ( dron[ind0].reg_dron == ind0 ) {
      ind0++;
    } else {
      numRef = ind0 ;
      ind0 = 200 ;
